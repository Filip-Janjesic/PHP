...
select ime from osoba order by ime desc limit 10,10; # prvih 10
...
# postaviti da su java programiranje i php programiranje tip Informatika

update smjer set tip='Informatika' where sifra in (1,2);


# postaviti da su ra?unalni operater i knjigovodstveni operater tip Ostalo
update smjer set tip='Ostalo' where sifra not in (1,2);

# koliko je ukupna suma cijena svih smjerova
select sum(cijena) from smjer;


select tip, sum(cijena), min(cijena),max(cijena),
avg(cijena),count(cijena),std(cijena) from smjer group by tip;


select tip, sum(cijena) from smjer group by tip having sum(cijena)>10000;

# podupiti

# islistajte sve smjerove koji nemaju niti jednu grupu
# s spajanjem tablica
select a.*
...
select * from smjer where sifra not in (select distinct smjer from grupa);
...
# iz tablice podaci obrišite Filip Gelenčir
delete from podaci where ime='Filip' and prezime='Gelenčir';
